...
\begin{figure}[h!]
  ...
  \begin{tikzpicture}[node distance=1cm and 1cm]
    ...
    \node (submit) [startstop, right=of conversion] {/submit [code]};
...
Para mejorar esta funcionalidad y ofrecer retroalimentación inmediata al estudiante, se propone implementar un sistema de evaluación automática de la correctitud de la solución. Actualmente, cuando el estudiante envía su código, este se almacena en la base de datos y se marca el ejercicio como resuelto. Sin embargo, con la propuesta, el sistema no solo almacenará el código del estudiante, sino que también lo ejecutará en un entorno controlado para verificar su correctitud. Este proceso incluirá la comparación de la salida generada por el código del estudiante con casos de prueba predefinidos, lo que permitirá identificar errores y proporcionar retroalimentación.
